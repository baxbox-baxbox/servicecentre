...
?>
	...
	<!-- end #header -->
		...
		<div style="clear: both;">
        			<div class="post">
     <br>  
   <div  style="font-size:22px; color:#000" align="center">Заявки клиента, <? echo $fio?></div>
      <br>  
...
$s="SELECT request.*, fio FROM request LEFT JOIN worker ON request.idworker=worker.idworker where request.idclient=$idclient and $fieldfind" ;
if ($value1!="Все")
...
$s=$s." and date>='$date1' and date<='$date2' ";
		 ...
		 $s="SELECT request.*, fio FROM request LEFT JOIN worker ON request.idworker=worker.idworker where request.idclient=$idclient ";
...
$s=$s." order by $fieldsort";
	 ...
	 ?>
     		...
     		<div align="right">	
Сортировка:
				<select name="sortname"  style="height:22; width:auto" onChange="this.form.action='reqclt.php?sort=1&filter=<? echo $filter;?>'; this.form.submit();" >
					...
					<option value="fio"  <? if ($fieldsort=="fio") {?> selected="selected" <? }?>>Исполнитель </option>	                    
...
по полю 
 &nbsp;  
				...
				<br>
				<input  type="button"  name="button1"  onclick="this.form.action='reqclt.php?filter=1&sort=<? echo $sort;?>'; this.form.submit();"   value="Фильтр">
				<input  type="button"  name="button2"  onclick="this.form.action='reqclt.php?filter=0&sort=<? echo $sort;?>'; this.form.submit();"   value="Очистить">
           <br>            
        </div>   
<?
 $r=mysql_query($s);
?>
 ...
 <input  type="button"  name="button4"   onclick="this.form.action='updreqclt.php?upd=0&step=1'; this.form.submit();" value="Добавить">
 <input  type="button"  name="button4" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>   onclick="this.form.action='updreqclt.php?upd=1&step=1'; this.form.submit();" value="Редактирование"> 
