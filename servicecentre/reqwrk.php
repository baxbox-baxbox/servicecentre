...
?>
	...
	<!-- end #header -->
		...
		<div style="clear: both;">
        			<div class="post">
     <br>  
   <div  style="font-size:22px; color:#000" align="center">Заявки исполнителю, <? echo $fio?></div>
      <br>  
...
$s="SELECT request.*, client.fio as clientfio FROM client, request, worker where  request.idworker=worker.idworker and request.idclient=client.idclient and request.idworker=$idworker and $fieldfind" ;
if ($value1!="Все")
...
$s=$s." and date>='$date1' and date<='$date2' ";
		 ...
		 $s="SELECT request.*, client.fio as clientfio FROM client, request, worker where  request.idworker=worker.idworker and request.idclient=client.idclient and request.idworker=$idworker ";
...
$s=$s." order by $fieldsort";
	 ...
	 ?>
     		...
     		<div align="right">	
Сортировка:
				<select name="sortname"  style="height:22; width:auto" onChange="this.form.action='reqwrk.php?sort=1&filter=<? echo $filter;?>'; this.form.submit();" >
...
по полю 
 &nbsp;  
				...
				<br>
				<input  type="button"  name="button1"  onclick="this.form.action='reqwrk.php?filter=1&sort=<? echo $sort;?>'; this.form.submit();"   value="Фильтр">
				<input  type="button"  name="button2"  onclick="this.form.action='reqwrk.php?filter=0&sort=<? echo $sort;?>'; this.form.submit();"   value="Очистить">
           <br>            
        </div>   
<?
 $r=mysql_query($s);
?>
 ...
 <input  type="button"  name="button4" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>   onclick="this.form.action='updreqwrk.php?upd=1&step=1'; this.form.submit();" value="Редактирование"> 
